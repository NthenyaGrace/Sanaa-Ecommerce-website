...
$sqlQ = "SELECT r.*, c.first_name, c.last_name, c.email, c.phone, c.address FROM orders as r LEFT JOIN users as c ON c.id = r.customer_id WHERE r.id=?"; 
        ...
        <?php if(!empty($orderInfo)){ ?>
            ...
            <div class="row col-lg-12">
                <table class="table table-hover cart">
                    ...
                    $sqlQ = "SELECT i.*, p.name, p.price FROM order_items as i LEFT JOIN gallery as p ON p.id = i.product_id WHERE i.order_id=?"; 
                    $stmt = $db->prepare($sqlQ); 
                    $stmt->bind_param("i", $db_id); 
                    $db_id = $order_id; 
                    $stmt->execute(); 
